...
SettingSchema.statics.getPublic = () => {
    return new Promise((resolve, reject) => {
        const query = Settings.findOne({});
        query.select('-_id report.enabled report.public reviews.enabled reviews.public llm.enabled llm.public');
        query.exec()
...
.then((liveSettings) => {
  if (!liveSettings) {
    ...
    // Check if LLM configuration is missing and add it
    if (!liveSettings.llm) {
      console.log("Adding LLM configuration to existing settings");
      liveSettings.llm = {
        enabled: false,
        public: {
          provider: 'openai',
